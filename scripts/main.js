...
const main = (() => {
    /**Module representing game board on user interface where the bulk of the
     * program is played.*/
    let currentUser = 'X';
    ...
    const toggleTiles = (isDisable) => {
        /**Toggles pointer-events CSS of all tiles in game board dependent on 
    ...
    const checkStatus = () => {
        ...
        } else if (!main.tiles.includes(0)) {
            _updateInterface(true);
        } else if (!winner && main.tiles.includes(0)){
            _statusDisplay.innerHTML = `It's ${main.currentUser}'s turn.`;
        }
    }

    const setUp = () => {
        /**Sets up the main module and displays its contents onto webpage.*/
        for (let i = 0; i < tiles.length; i++) {
